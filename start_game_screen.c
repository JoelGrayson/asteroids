...
void start_game_screen() {
    while (waiting) {
        ...
        gl_draw_string(MONITOR_WIDTH / 2 - 190, MONITOR_HEIGHT * 0.4, "Press any button to start", GL_WHITE);

        gl_draw_string(MONITOR_WIDTH / 2 - 120, MONITOR_HEIGHT * 0.45, "High Scores", GL_WHITE);
        ...
        for(int i = 0; i < 10; i++) {
            ...
            if(score > -1) {
                ...
                gl_draw_string(MONITOR_WIDTH / 2 - 130, MONITOR_HEIGHT * (0.50 + ((double)i)*0.05), printbuf, GL_WHITE);
            }
        }
