...
void main(void) {
    ...
    //assert(gwav_saucerBig->bits_per_sample == 16); // DAC support only tested for 16-bit
    int sample_freq = gwav_saucerBig->frames_per_sec;
    i2s_init(sample_freq);
    i2s_frame_type_t ftype = (gwav_saucerBig->samples_per_frame == 1) ? I2S_MONO : I2S_STEREO;

    i2s_stream_playback_nb(gwav_saucerBig->data, gwav_saucerBig->n_data, ftype);
    i2s_await();
